...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        else if(contestRepository.findById(contestId).isEmpty()) {
            throw new RuntimeException("Contest with id : " + contestId + " does not exist!");
        } else {
            Contest contest = contestRepository.findById(contestId).get();
            if(contest.getContestStatus() == ContestStatus.IN_PROGRESS || contest.getContestStatus() == ContestStatus.ENDED) {
                throw new RuntimeException("Error adding user " + userName + "to contest with id : " + contestId + ". Contest has already started/ended.");
            } else {
                User user = userRepository.findByName(userName).get();                
                if(contestantRepository.find(contestId, userName).isEmpty()) {
                    Contestant contestant = new Contestant(user, contest);
                    return contestantRepository.save(contestant);
                } else {
                    System.out.println("User  " + user + " already exist in Contest " + contest);
                    return null;
                } 
            }
        }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        if(contest.getContestStatus() == ContestStatus.NOT_STARTED) {
            throw new RuntimeException("Error while fetching contestHistory. Contest " + contest + " never ran.");
        }
        List<Contestant> contestantList = contestantRepository.findAllByContestId(contestId);
        return contestantList.stream().sorted((a,b) -> Integer.compare(b.getCurrentContestPoints(),a.getCurrentContestPoints())).collect(Collectors.toList());
    } 
}
